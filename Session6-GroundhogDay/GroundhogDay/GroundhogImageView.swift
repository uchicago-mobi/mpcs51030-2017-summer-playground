...
class GroundhogImageView: UIImageView {
  ...
  override init (frame : CGRect) {
    super.init(frame : CGRect(x: 200, y: 1000, width: 300, height: 300))
    self.image = UIImage(named: "Pikachu")
    self.tag = 102
    
    let singeTap = UITapGestureRecognizer(target: self, action: #selector(GroundhogImageView.doubleTapField(_:)))
